...
public interface UserRepository  extends CrudRepository<User, Integer> {
    ...
    @Modifying
    @Query(value = "UPDATE users SET verified=1, token=null, verified_at=NOW(), updated_at=NOW() WHERE " +
            "email= :email", nativeQuery = true)
    @Transactional
    void verifyAccount(@Param("email")String email);
    @Query(value = "SELECT token FROM users WHERE email = :email" , nativeQuery = true)
    String checkToken(@Param("email")String email);

    @Query(value = "SELECT email FROM users WHERE token = :token" , nativeQuery = true)
    String checkEmail(@Param("token")String token);
